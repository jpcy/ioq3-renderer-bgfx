...
namespace shaderc
{
	...
	bool getReflectionDataDx9(CallbackI* _callback, ID3DBlob* _code, UniformArray& _uniforms)
	{
		...
		while (ptr < end && *ptr != D3DSIO_END)
		{
			...
			if (fourcc == CTAB_CONSTANT)
			{
				...
				{
					writeLog(_callback, "Error: Invalid constant table data\n");
					return false;
				}
				break;
			}
		...
		if (!header)
		{
			writeLog(_callback, "Error: Could not find constant table data\n");
			return false;
		}
	...
	bool getReflectionDataDx11(CallbackI* _callback, ID3DBlob* _code, bool _vshader, UniformArray& _uniforms, uint8_t& _numAttrs, uint16_t* _attrs, uint16_t& _size, UniformNameList& unusedUniforms)
	{
		...
		if (FAILED(hr) )
		{
			writeLog(_callback, "Error: 0x%08x\n", (uint32_t)hr);
			return false;
		}
		...
		if (FAILED(hr) )
		{
			writeLog(_callback, BX_FILE_LINE_LITERAL "Error: 0x%08x\n", (uint32_t)hr);
			return false;
		}
	...
	static const char* s_profileVersionName[] =
	{
		"3_0",
		"4_0",
		"4_1",
	...
	bool compileHLSLShader(const CompileShaderParameters& _params, uint32_t _d3d, const std::string& _code, FileWriter* _writer, bool _firstPass)
	{
		BX_TRACE("DX11");

		char profile[16];
		if (_params.profile >= Profile::HLSL3 && _params.profile <= Profile::HLSL5)
		{
			bx::snprintf(profile, sizeof(profile), "%cs_%s", "cpv"[_params.shaderType], s_profileVersionName[_params.profile - Profile::HLSL3]);
		}
		else
		{
			writeLog(_params.callback, "Invalid HLSL shader profile %d.\n", _params.profile);
			return false;
		}

		const bool debug = (_params.flags & Flags::D3DDebug) != 0;

		uint32_t flags = D3DCOMPILE_ENABLE_BACKWARDS_COMPATIBILITY;
		flags |= debug ? D3DCOMPILE_DEBUG : 0;
		flags |= (_params.flags & Flags::D3DAvoidFlowControl) ? D3DCOMPILE_AVOID_FLOW_CONTROL : 0;
		flags |= (_params.flags & Flags::D3DNoPreshader) ? D3DCOMPILE_NO_PRESHADER : 0;
		flags |= (_params.flags & Flags::D3DPartialPrecision) ? D3DCOMPILE_PARTIAL_PRECISION : 0;
		flags |= (_params.flags & Flags::D3DPreferFlowControl) ? D3DCOMPILE_PREFER_FLOW_CONTROL : 0;
		flags |= (_params.flags & Flags::D3DBackwardsCompatibility) ? D3DCOMPILE_ENABLE_BACKWARDS_COMPATIBILITY : 0;

		const bool werror = (_params.flags & Flags::D3DWarningsAsErrors) != 0;
		...
		uint32_t optimization = _params.d3dOptimizationLevel;
		if (_params.flags & Flags::D3DOptimize)
		{
		...
		if (debug && NULL != _params.outputFilePath)
		{
			hlslfp = _params.outputFilePath;
			hlslfp += ".hlsl";
			writeFile(_params.callback, hlslfp.c_str(), _code.c_str(), (int32_t)_code.size() );
		}
		...
		|| (werror && NULL != errorMsg) )
		{
			...
			printCode(_params.callback, _code.c_str(), line, start, end);
			writeLog(_params.callback, "Error: 0x%08x %s\n", (uint32_t)hr, log);
			errorMsg->Release();
			return false;
		}
		...
		if (_d3d == 9)
		{
			if (!getReflectionDataDx9(_params.callback, code, uniforms) )
			{
				return false;
			}
		}
		else
		{
			...
			if (!getReflectionDataDx11(_params.callback, code, profile[0] == 'v', uniforms, numAttrs, attrs, size, unusedUniforms) )
			{
				return false;
			}
			...
			&&  unusedUniforms.size() > 0)
			{
				...
				return compileHLSLShader(_params, _d3d, output.c_str(), _writer, false);
			}
		}
		...
		for (UniformArray::const_iterator it = uniforms.begin(); it != uniforms.end(); ++it)
		{
			...
			_writer->write(nameSize);
			_writer->write(un.name.c_str(), nameSize);
			uint8_t type = un.type|fragmentBit;
			_writer->write(type);
			_writer->write(un.num);
			_writer->write(un.regIndex);
			_writer->write(un.regCount);
		...
		if (_d3d > 9)
		{
			_writer->write(numAttrs);
			_writer->write(attrs, numAttrs*sizeof(uint16_t) );

			_writer->write(size);
		}

		if ((_params.flags & Flags::D3DDisassemble) && NULL != _params.outputFilePath)
		{
			ID3DBlob* disasm;
			...
			if (NULL != disasm)
			{
				std::string disasmfp = _params.outputFilePath;
				disasmfp += ".disasm";

				writeFile(_params.callback, disasmfp.c_str(), disasm->GetBufferPointer(), (uint32_t)disasm->GetBufferSize() );
				disasm->Release();
			}
		}
...
namespace shaderc
{
	bool compileHLSLShader(ShaderType::Enum _shaderType, const char* _outputPath, const CompileOptions& _options, uint32_t _d3d, const std::string& _code, bx::WriterI* _writer, bool _firstPass)
	{
		BX_UNUSED(_shaderType, _outputPath, _options, _d3d, _code, _writer, _firstPass);
		writeLog(_params.log, "HLSL compiler is not supported on this platform.\n");
		return false;
	}
} // namespace shaderc
