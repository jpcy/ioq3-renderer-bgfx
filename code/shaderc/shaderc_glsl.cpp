...
namespace shaderc
{
	...
	{
		const glslopt_shader_type type = _params.shaderType == ShaderType::Fragment
			? kGlslOptShaderFragment
			: (_params.shaderType == ShaderType::Compute ? kGlslOptShaderCompute : kGlslOptShaderVertex);
		...
		if (!glslopt_get_status(shader) )
		{
			...
			writeLog(_params.callback, "Error: %s\n", log);
			glslopt_cleanup(ctx);
			return false;
		}
		...
		_writer->write(optimizedShader, shaderSize);
		uint8_t nul = 0;
		_writer->write(nul);
